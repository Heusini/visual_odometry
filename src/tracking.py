...
class Tracking:
    ...
    def check_for_new_landmarks(self, frame_states: FrameState, K: np.ndarray) -> bool:
        for start_frame_index in self.start_frame_indices:
            ...
            c_P_start = K_inv @ homogenous_start_track
            c_P_end = K_inv @ homogenous_end_track

            dot_products = np.diag(np.dot(c_P_start.T, c_P_end))
            start_norms = np.linalg.norm(c_P_start, axis=0)
            end_norms = np.linalg.norm(c_P_end, axis=0)
            ...
            p_start = homogenous_start_track[:, mask_angle]
            p_end = homogenous_end_track[:, mask_angle]
            ...
            p_start = p_start[:, mask_filter]
            p_end = p_end[:, mask_filter]

            P_world = cam_start_to_world @ P_cam_start
