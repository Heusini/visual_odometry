...
def pnp(points_2d: np.ndarray, points_3d: np.ndarray, K: np.ndarray):
    ...
    success, rotation_vector_world_camj, translation_world_camj, mask_ransac = (
        cv.solvePnPRansac(
            ...
            iterationsCount=params.CONT_PARAMS.PNP_RANSAC_PARAMS.NUM_ITERATIONS,
            reprojectionError=params.CONT_PARAMS.PNP_RANSAC_PARAMS.THRESHOLD,
            confidence=params.CONT_PARAMS.PNP_RANSAC_PARAMS.CONFIDENCE,
        )
    )
